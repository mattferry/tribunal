...
## The four receipts — an audit is real only if all four are in the report

1. **It ran.** The exact invocation and its raw or structured output appear in the report (or
   a saved file whose path is given). An audit narrated from memory scores as not-run — even
   when it really ran. Baseline arms did the work and could not prove it; proof is part of
   the work.
2. **It was caged.** Read-only enforcement is a **flag, not a vibe**: Grok
   ...
   read-only by construction. A prompt sentence like "do not modify any files" is a request
   to a write-capable agent, not a control — and `--permission-mode auto`/`--always-approve`
   on an auditor is the opposite of a control. Never write the words "read-only" in a report
   unless the shown command proves it.
3. **You verified it.** Every finding gets a disposition you produced by reading the actual
   code — CONFIRMED (with the line and the reason), REFUTED (with the reason), or UNCERTAIN
   (with what would settle it). Show at least the decisive evidence for ship-blockers; "I
   verified each against the code" with nothing shown is an assertion, not verification.
4. **Costs and gaps are named.** What was spent ("2 Grok calls, 1 local"), and which panel
   members didn't run and why ("Codex unavailable: no auth"). A silently shrunken panel is a
   faked panel.
...
1. Number the auditor's findings exactly as returned; keep its severities as *its* ratings.
...
- A panel member missing from the report that was supposed to run.
